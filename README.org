...
Just like we installed babel loader to connect babel to WebPack, we need a 
loader for our CSS as well so that WebPack knows to transpile our css. If we
want to use sass, then we need a sass loader ontop of that. If we want to load
images in our React app, then we need a file-loader. 
